...
From MetaRocq.PCUIC Require Import PCUICAst PCUICTyping PCUICReduction PCUICAstUtils PCUICSN PCUICTyping PCUICProgram PCUICFirstorder PCUICEtaExpand.
...
Record good_for_extraction (fl : EWellformed.EEnvFlags) (p : program (list (kername × EAst.global_decl)) EAst.term) :=
    ...
    forall (i : inductive) (mb : EAst.mutual_inductive_body)
		...
		#|EAst.ind_ctors ob| < Z.to_nat Numbers.Cyclic.Int63.Uint63.wB ;
...
| (kn, EAst.InductiveDecl mind) :: Σ =>
	...
	&|&
	forallb (fun ob => Z.of_nat #|EAst.ind_ctors ob| <? Numbers.Cyclic.Int63.Uint63.wB)%Z mind.(EAst.ind_bodies) >>> "inductive with too many constructors"
	&|&
...
Definition compile_malfunction_gen config (p : Ast.Env.program) : list string * string := (* Exported names, code *)
	let nms := extract_names p.2 in
	let p' := run (malfunction_pipeline config) (config.(reorder_cstrs), p) (trust_coq_kernel config p) in
	let code := time "Pretty printing" (fun p_c => @CeresSerialize.to_string _ (Serialize_module (rev nms)) p_c) p' in
	(nms, code).
